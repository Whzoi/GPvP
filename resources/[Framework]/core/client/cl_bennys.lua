...
local bennysLocations = {
    { center = vector3(247.9544, -1403.0729, 30.5875), radius = 5.0, minZ = 27.08866, maxZ = 30.10112 },
    { center = vector3(-1144.8097, -211.0682, 37.9598), radius = 5.0, minZ = 27.08866, maxZ = 30.10112 },
    { center = vector3(1029.7314, -772.8080, 58.0501), radius = 5.0, minZ = 56.0, maxZ = 59.0 },
    { center = vector3(-876.5094, -858.5177, 19.1232), radius = 5.0, minZ = 18.0, maxZ = 20.0 },
    { center = vector3(1968.1569, 3700.7034, 32.3351), radius = 5.0, minZ = 18.0, maxZ = 20.0 },
    { center = vector3(74.9839, 122.3576, 79.2040), radius = 3.0, minZ = 18.0, maxZ = 20.0 },
    { center = vector3(-876.5094, -858.5177, 19.1232), radius = 5.0, minZ = 18.0, maxZ = 20.0 },
}

local inside = false

-- Register the polyzone
CreateThread(function()
    for i, location in ipairs(bennysLocations) do
        exports["core"]:AddCircleZone(
            "bennys"..i,
            location.center,  -- Center of the circle
            location.radius,  -- Radius of the circle
            {
                minZ = location.minZ,
                maxZ = location.maxZ,
            }
        ...
        -- Create checkpoints for each Benny's location
        local checkpoint = CreateCheckpoint(5, location.center.x, location.center.y, location.center.z - 1.0, location.center.x, location.center.y, location.center.z - 1.0, location.radius, 255, 255, 255, 100, 0)
        SetCheckpointCylinderHeight(checkpoint, 1.0, 3.0, 0.0)
    end
end)

-- Event when entering the polyzone
local interactCheck = false
local inside = false
...
AddEventHandler("polyzone:enter", function(name)
    if string.sub(name, 1, 6) == "bennys" then
        local ped = PlayerPedId()
        if not inside and IsPedSittingInAnyVehicle(ped) then
