local clothingShops = {
    {coords = vector3(214.3648, -1386.4623, 30.5875), blip = true, job = 'all'},
}

local barberShops = {
    {coords = vector3(216.2395, -1388.5682, 30.5875), blip = true},
}

local tattoosShops = {
    {coords = vector3(219.2240, -1391.0483, 30.5875), blip = true},
}
...
function Save(save)
    if save then
        ...
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            ...
            SetResourceKvp("character_current-gpvp", json.encode(data))
            SetResourceKvp("character_face-gpvp", json.encode(data))
            SetResourceKvp("playersTattoos-gpvp", json.encode(currentTats))
        end
    else
        LoadPed(oldPed)
    end
...
AddEventHandler("drp_clothing:retrieve_tats", function()
    local kvpValue = GetResourceKvpString("playersTattoos-gpvp")
    if kvpValue then
        local data = json.decode(kvpValue)
        TriggerEvent("drp_clothing:settattoos-gpvp", data)
    else
        TriggerEvent("drp_clothing:settattoos-gpvp", {})
    end
end)
...
AddEventHandler("drp_clothing:setclothes", function(data,alreadyExist,firstspawn, kvpArmour, kvpStress, kvpThirst, kvpHunger)
    ...
    local kvpValue = GetResourceKvpString("character_face-gpvp")
    if kvpValue then
        local data = json.decode(kvpValue)
...
AddEventHandler('clothing:checkIfNew', function()
    local kvpValue = GetResourceKvpString("character_current-gpvp")
    if kvpValue then
...
local peds = {
	{
	  	coords = vector3(214.3648, -1386.4623, 30.5875),
	  	heading = 320.5612,
	  	pedModel = "a_f_y_fitness_02",
        scenario = "",
	},
    {
        coords = vector3(216.2395, -1388.5682, 30.5875),
        heading = 320.5612,
    ...
    {
        coords = vector3(219.2240, -1391.0483, 30.5875),
        heading = 320.5612,
        pedModel = "A_C_Boar",
        scenario = "",
    },
  ...
  InteractWithPed = function(self, ped)
    ...
    for _, pedEntity in pairs(lib.entities) do
      if DoesEntityExist(pedEntity) then
        ...
        if #(playerCoords - peds.coords) < 2.0 then
