droppedItems = {}
local dropModelCache = {}

local function ensureModel(model)
    if dropModelCache[model] then
        return true
    end
    RequestModel(model)
    ...
    for k,v in pairs(droppedItems) do
        ...
        if item.itemdata and item.itemdata.model then
            ...
            if IsModelValid(model) and IsModelInCdimage(model) then
                ...
                if droppedItems[k] and droppedItems[k].objecthandle and DoesEntityExist(droppedItems[k].objecthandle) then
                    FreezeEntityPosition(droppedItems[k].objecthandle, true)
                end
            end
        end
    end
...
AddEventHandler('zbrp:addDroppedItem', function(id, data)
    ...
    if item.itemdata and item.itemdata.model then
        ...
        if IsModelValid(model) and IsModelInCdimage(model) then
            ...
            if droppedItems[id] and droppedItems[id].objecthandle and DoesEntityExist(droppedItems[id].objecthandle) then
