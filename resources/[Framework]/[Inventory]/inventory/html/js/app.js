...
const app = new Vue({
    ...
    data: {
        ...
        secondInventoryStaticData: {
            ...
            hideHeader: false,
            showWeight: true
        }, // not exactly static, because we are changing it every time.
        ...
        settings: {
            ...
            setters: {
                ...
                blur: {
                    ...
                    checkbox: true,
                    set: localStorage['blur'] == undefined ? 'off' : localStorage['blur'],
                    default: 'off'
                },
                itemdescription: {
                ...
                itembg_1: {
                    ...
                    color: true,
                    set: localStorage['itembg_1'] == undefined ? 'rgba(180, 180, 180, 0.35)' : localStorage['itembg_1'],
                    default: 'rgba(180, 180, 180, 0.35)'
                },
                itembg_2: {
                    name: _U('settings_itembg_2'),
                    color: true,
                    set: localStorage['itembg_2'] == undefined ? 'rgba(30, 30, 30, 0.70)' : localStorage['itembg_2'],
                    default: 'rgba(30, 30, 30, 0.70)'
                },
                weightbg_1: {
                    name: _U('settings_weightbg_1'),
                    color: true,
                    set: localStorage['weightbg_1'] == undefined ? '#2a2a2a' : localStorage['weightbg_1'],
                    default: '#2a2a2a'
                },
                weightbg_2: {
                    name: _U('settings_weightbg_2'),
                    color: true,
                    set: localStorage['weightbg_2'] == undefined ? '#ff9dbb' : localStorage['weightbg_2'],
                    default: '#ff9dbb'
                }
            }
        },
    ...
    methods: {
        ...
        getImage(itemData) {
            if (itemData.image) {
                if (itemData.type && itemData.type === 'kit') {
                    return 'img/items/kits/' + itemData.image + '.png'
                }
...
window.addEventListener("message", (event) => {
    if (event.data.action == 'openInventory') {
        ...
        self.secondInventoryStaticData.name = event.data.name || null;
        self.secondInventoryStaticData.hideHeader = event.data.hideHeader || false;
        self.secondInventoryStaticData.showWeight = (event.data.showWeight === undefined || event.data.showWeight === null) ? true : event.data.showWeight;
    }
