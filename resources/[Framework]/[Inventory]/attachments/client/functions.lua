    ...
    if gunHash ~= nil then
        for i, v in pairs(WEAPON_LIST[gunHash]) do
            if v.bone then
                ...
                -- Apply offset if specified (takes priority over label-based offsets)
                if v.offset then
                    bonePosition = GetOffsetFromEntityInWorldCoords(EDIT_GUN, v.offset.x or 0, v.offset.y or 0, v.offset.z or 0)
                elseif v.label == "Tint" then
                    bonePosition = GetOffsetFromEntityInWorldCoords(EDIT_GUN, 0.03, 0, -0.05)
                elseif v.label == "Variation" then
                ...
                if onScreen then
                    local dx = (tonumber(_x:sub(1, -2)) or 50) - 50
                    local dy = (tonumber(_y:sub(1, -2)) or 50) - 50
...
function prewarmAttachmentModels(gunHash)
    if not gunHash or not WEAPON_LIST[gunHash] then return end
    CreateThread(function()
        for i = 1, #WEAPON_LIST[gunHash] do
            local category = WEAPON_LIST[gunHash][i]
            if category and category.options then
                for y = 1, #category.options do
                    local opt = category.options[y]
                    if opt.attachment and type(opt.attachment) == "string" and not opt.attachment:find("CLIP") then
                        local compHash = joaat(opt.attachment)
                        local model = GetWeaponComponentTypeModel(compHash)
                        if model and model ~= 0 then
                            RequestModel(model)
                            local tries = 0
                            while not HasModelLoaded(model) and tries < 25 do
                                Wait(0)
                                tries += 1
                            end
                        end
    ...
    for i = 1, #weaponListCopy[gun] do
        if weaponListCopy[gun][i] then
            ...
            if weaponListCopy[gun][i].label == "Tint" then
                ...
                -- Check if weapon has custom tints defined
                local customTints = weaponListCopy[gun][i].customTints
                local tintList = customTints or nil
                
                if not tintList then
                    -- Fall back to default tint system
                    local tintCount = GetWeaponTintCount(gun)
                    local tintIndex = (tintCount < 9 and 1) or 2
                    tintList = WEAPON_TINTS[tintIndex]
                    
                    -- Default system: use array index as tint index
                    for tint, data in ipairs(tintList) do
                    ...
                    -- Custom tints: use tintIndex field if specified, otherwise use array index (0-based)
                    for tint, data in ipairs(tintList) do
                        local tintIndex = data.tintIndex ~= nil and data.tintIndex or (tint - 1) -- Use tintIndex if specified (0-based), otherwise use array position converted to 0-based
                        local selected = GetPedWeaponTintIndex(PlayerPedId(), gun) == tintIndex
                        weaponListCopy[gun][i].options[#weaponListCopy[gun][i].options + 1] = {price = selected and "Equipped" or "", label = data.label, attachment = tintIndex + 1, selected = selected}
                    end
                end
            else
...
function refreshEditGun(gunHash)
    ...
    while not DoesEntityExist(EDIT_GUN) do
        if EDIT_GUN == 0 or EDIT_GUN == nil then
            ...
            SetEntityRotation(EDIT_GUN, 90.0, 0.0, gunLastCoords.w, 2) -- lay flat, flipped
...
function loadGun(gunHash, coords)
    ...
    SetEntityRotation(EDIT_GUN, 90.0, 0.0, coords.w or 0.0, 2) -- lay flat and respect yaw, flipped
    FreezeEntityPosition(EDIT_GUN, true)
